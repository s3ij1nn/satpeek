...
<section class="viewer">
    <header class="viewer__head">
        ...
        <div class="viewer__meta">reward: <span style="color: var(--amber-soft);">{{ number_format($ad->reward_sat) }} sat</span></div>
    </header>

    <div class="frame {{ $ad->display_mode === 'iframe' ? 'frame--iframe' : '' }}">
        <div class="frame__bar">
            <span class="url" title="{{ $ad->target_url }}">{{ $ad->target_url }}</span>
            <span id="ptcOpenStatus" class="viewer__meta">{{ $ad->display_mode === 'iframe' ? 'loading' : 'awaiting open' }}</span>
        </div>
        <div class="frame__body">
            @if ($ad->display_mode === 'iframe')
                {{-- Inline iframe — only works when the destination doesn't set
                     X-Frame-Options/CSP frame-ancestors. The countdown still
                     runs in this tab; if the embedded site top-frame-redirects
                     it'll navigate the whole viewer away (the failure case
                     that motivated the `window` mode). --}}
                <iframe id="ptcIframe"
                        src="{{ $ad->target_url }}"
                        sandbox="allow-scripts allow-same-origin allow-popups allow-forms"
                        referrerpolicy="no-referrer"
                        loading="lazy"
                        title="{{ $ad->title }}"></iframe>
            @else
                @if ($ad->description)
                    <p class="frame__title">{{ $ad->description }}</p>
                @else
                    <p class="frame__title">{{ $ad->title }}</p>
                @endif
    ...
    <p style="font-size: var(--text-xs); color: var(--text-tertiary); text-align: center; margin: 0;">
        ← <a href="{{ route('ptc.index') }}" style="color: var(--text-secondary); text-decoration: underline;">Back to PTC list</a>
    </p>
</section>
...
(() => {
    ...
    const targetUrl = @json($ad->target_url);
    const duration = {{ (int) $ad->duration_sec }};
    const displayMode = @json($ad->display_mode);
    ...
    claim.addEventListener('click', async () => {
        ...
        try {
            ...
            setTimeout(() => location.href = '{{ route('ptc.index') }}', 1400);
        } catch (e) {
